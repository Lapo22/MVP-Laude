...
type TeamVotesTableProps = {
  teams: TeamStat[];
  onViewDetails: (id: string, name: string) => void;
};

const TeamVotesTable = ({ teams, onViewDetails }: TeamVotesTableProps) => {
          ...
          <tbody className="divide-y divide-gray-100">
            {teams.map((team) => (
              <tr key={team.id} className="hover:bg-gray-50 transition-colors">
                <td className="py-2.5 text-sm font-medium text-gray-900">{team.name}</td>
                <td className="py-2.5 text-sm text-gray-700">{team.totalVotes}</td>
                <td className="py-2.5 text-sm text-gray-700">
                  {team.averageRating !== null ? (
                    <span className="font-medium">{team.averageRating.toFixed(1)}</span>
                  ) : (
                    <span className="text-gray-400">–</span>
                  )}
                </td>
                <td className="py-2.5 text-sm text-gray-600">{formatDate(team.lastVoteAt)}</td>
                <td className="py-2.5">
                  <button
                    type="button"
                    onClick={() => onViewDetails(team.id, team.name)}
